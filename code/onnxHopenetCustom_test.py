...
if __name__ == '__main__':
    ...
    # Load ONNX model
    onnx_model_path = args.snapshot
    onnx_model = onnx.load(onnx_model_path)
    csv_file_path = os.path.split(onnx_model_path)[1].split('.')[0]
    ...
    for i, (images, name) in enumerate(test_loader):
        ...
        if args.save_viz:
            ...
            os.makedirs(f'C:\\Users\\Girish\\Desktop\\FF-Projects\\deep-head-pose\\output\\{csv_file_path}_ONNX', exist_ok=True)
            ...
            plt.savefig(os.path.join(f'C:\\Users\\Girish\\Desktop\\FF-Projects\\deep-head-pose\\output\\{csv_file_path}_ONNX', os.path.split(name)[-1] + '.jpg'), bbox_inches='tight')
            plt.close()
    ...
    # print(df_common_paths)
    # df_common_paths.to_csv(f'.\\output\\Multi-PIE_samples.csv', index=False)
    # df_common_paths.to_csv(f'C:\\Users\\Girish\\Desktop\\FF-Projects\\MatchScoreAnalysis\\NexDataCropped\\femalecaucassian01719HP.csv', index=False)
